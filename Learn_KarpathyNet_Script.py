...
parser = argparse.ArgumentParser(description='KarpathyNet Training Script')
...
class DatasetCifar(Dataset):
    def __init__(self , X , y , device = device, img_shape = (3,32,32)):
        # self.X , self.y = self.augmentation(X , y, pixels=2)
        self.X,self.y = X, y
        self.labels = torch.sort(torch.unique(y))
...
train_dataset = DatasetCifar(Xtr , ytr)
len(train_dataset)
