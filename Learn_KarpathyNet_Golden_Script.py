...
# * **Title** : Learn KarpathyNet Golden Model (ie Model without forget set classes)
...
parser = argparse.ArgumentParser(description='KarpathyNet CIFAR-10 Golden Model Training \nNote : forget classes are hard coded as 3,4,8 change in script if need to experiment on different set ')
...
def remove_class(X,y, target_classes):
    mask = np.isin(y, target_classes, invert=True)
    print(f'[+] Removing classes {target_classes} from dataset')
    X = X[mask]
    y = y[mask]
    return X,y

cifar_train_data, cifar_train_labels = remove_class(cifar_train_data, cifar_train_labels, list(Target_classes))
# cifar_train_data.shape, cifar_train_labels.shape
...
Xte , yte = remove_class(Xte, yte, list(Target_classes))

# print(f'[>] {Xtr.shape = },{ ytr.shape = }, {Xval.shape = }, {yval.shape = }, {Xte.shape = }, {yte.shape = }')
...
labels_new = set(labels) - Target_classes
